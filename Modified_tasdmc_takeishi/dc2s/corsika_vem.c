...
//double corsika_vem(char *filename, char *tmpfile, int tcount, int np, double avem[nsubblock][40][2])
double corsika_vem(char *filename, char *tmpfile, int tcount, int np, double avem0[NSUBBLOCK][40], double avem1[NSUBBLOCK][40])
{
  int i, j, k, m, n;
  FILE *f, *fout;
  short pz_tmp;
...
//fprintf(stdout, "\nnp: %d, buf3: %f, int_buf3: %d\n\n", np, buf3[0], (int)(buf3[0]/1000));
//// empty for atmos
  if(np==5){
  }
//// gamma
