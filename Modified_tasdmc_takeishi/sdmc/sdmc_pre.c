...
int main(int argc, char **argv)
{
  ...
  while ( nEvent_global > 0 )
      ...
      for (i = 0; i < nEvent; i++)
	{
      ...
      rusdmc_.energy = eventbuf[3]/1.e9;
      for (i = 0; i < nEvent; i++) encor[i] = 1.;
      rusdmc_.height = eventbuf[6];
      rusdmc_.theta = eventbuf[10];
      rusdmc_.phi = eventbuf[11]+MAGDEC;
      if ( rusdmc_.phi < 0. ) rusdmc_.phi += 2.*PI;
      else if ( rusdmc_.phi >= 2.*PI ) rusdmc_.phi -= 2.*PI;
