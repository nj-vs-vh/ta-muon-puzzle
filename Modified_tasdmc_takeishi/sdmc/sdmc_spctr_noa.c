...
int main(int argc, char **argv)
{
	...
	////
	char str1[] = "ABCDEFGHI";
	strncpy(str1, argv[1], 9);
	char infile[256];
	//  int np = 1, en[5][NT], aa, trflg[5][NT][NCOUNTERS][FADC_TOT/2][NCOUNTERS], st[5][NT][NSECT], kk;
	int np = 1, en[5][NT], aa, trflg[5][NT][NCOUNTERS][FADC_TOT / 2], st[5][NT][NSECT], kk, sdflg[5][NT][NCOUNTERS], nwf[5][NCOUNTERS];
	//  int wfn, trtflg[5][NT][NCOUNTERS][FADC_TOT/2];
	const int npmax = 6;
	float arand[5][NT][4], aencor[5][NT], atime[5][NT], brand[3][NT][TMAX][NCOUNTERS][2], crand[3][NT][NCOUNTERS][TMAX][2];
	float atable[3][NT][NCOUNTERS][FADC_TOT / 2];
	float m_rand[3][NT][TMAX][NCOUNTERS][2], t_rand[3][NT][TMAX][NCOUNTERS], am_rand[3][NT][TMAX][NCOUNTERS][2], at_rand[3][NT][TMAX][NCOUNTERS];
	float amev_t, faflg[FADC_TOT][TMAX];
	int jj, b4[3][NT][100][NCOUNTERS], jm[3][NT][NCOUNTERS];
...
*/
		...
		////
		printf("dataset: %d\n", dataset);
		sprintf(infile, "%s_%d.p%02d", str1, np, dataset);
		printf("infile: %s\n", infile);
		sprintf(outFile, "%s_%d.p%02d.dst.gz", str1, np, dataset);
		printf("outFile: %s\n", outFile);
		if ((rc = dstOpenUnit(outUnit, outFile, outMode)) < 0)
		{
			fprintf(stderr, "Can't start the dst file\n");
	...
	printf("%f ", mftndof[i][j][1]);
      printf("\n");
		...
		i = 0;
		while (fread(atmos[i], sizeof(float), 2, f) == 2 && i < NATMOS)
			i++;
		fclose(f);
		if (np == 0)
			natmos = i;
		...
		while (nEvent_global > 0)
		{
			...
			for (i = 0; i < nEvent; i++)
			{
				if (np == 0)
				{
					...
					arand[aa][i][0] = randnum[0];
					arand[aa][i][1] = randnum[1];
					arand[aa][i][2] = randnum[2];
					arand[aa][i][3] = randnum[3];
				}
				else
				{
					randnum[0] = arand[aa][i][0];
					randnum[1] = arand[aa][i][1];
...
////
if(np==0){
  am_rand[aa][i][j][k][0] =  mev[j][i][k][0];
  am_rand[aa][i][j][k][1] =  mev[j][i][k][1];
  at_rand[aa][i][j][k] = time[j][i][k];
}
else{
  mev[j][i][k][0] = am_rand[aa][i][j][k][0];
  mev[j][i][k][1] = am_rand[aa][i][j][k][1];
  time[j][i][k] = at_rand[aa][i][j][k];
}
////
*/
							}
			...
			while (fread(buf, sizeof(short), 6, f) == 6)
			{
				...
				for (i = 0; i < nEvent; i++)
				{
					if (sdmap[m][n][i] > 0)
					{
						...
						printf("-- sdmap > 0 - np:%d, nevt_global:%d, nevt:%d, sdmap:%d, buf0:%d, buf1:%d, buf2:%f, buf3:%f, buf4:%d, buf5:%f\n", np, aa, i, sdmap[m][n][i], (int)buf[0], (int)buf[1], (float)buf[2], (float)buf[3], (int)buf[4], (float)buf[5]);
						...
						j = 0;
						while (time[sdmap[m][n][i]][i][j] > -1000000 && j < TMAX)
							j++;
						if (j < TMAX)
						{
							if (np == 0)
							{
								gaussf(randnum);
								brand[aa][i][j][sdmap[m][n][i]][0] = randnum[0];
								brand[aa][i][j][sdmap[m][n][i]][1] = randnum[1];
								b4[aa][i][j][sdmap[m][n][i]] = (int)buf[4];
								jm[aa][i][sdmap[m][n][i]] = j;
								printf("-- b4 -- nevt_global:%d, nevt:%d, nsd: %d, sdmap:%d, b4:%d, jmax:%d\n", aa, i, j, sdmap[m][n][i], b4[aa][i][j][sdmap[m][n][i]], jm[aa][i][sdmap[m][n][i]]);
							}
							else
							{
								randnum[0] = brand[aa][i][j][sdmap[m][n][i]][0];
								randnum[1] = brand[aa][i][j][sdmap[m][n][i]][1];
								printf("-- b4 -- nevt_global:%d, nevt:%d, nsd: %d, sdmap:%d, b4:%d, jmax:%d\n", aa, i, j, sdmap[m][n][i], b4[aa][i][j][sdmap[m][n][i]], jm[aa][i][sdmap[m][n][i]]);
							}

							////
							//  printf("nevt_global:%d, nevt:%d, nsd: %d, sdmap:%d, brand1:%f, brand2:%f\n", aa, i, j, sdmap[m][n][i], randnum[0], randnum[1]);
							////
...
/*if(np==0){
  ...
  poisson_tmp = m_rand[aa][i][j][sdmap[m][n][i]][0];
}*/
							/*		      if (poisson_tmp > 0.)
			poisson_tmp *= 1.+ NPE2FADCE[0]/sqrtf(poisson_tmp)*
			  randnum[0];
...
*/
							if (np == 0)
							{
								...
								if (amev_t == 0)
									...
									m_rand[aa][i][j][sdmap[m][n][i]][0] = mev[sdmap[m][n][i]][i][j][0] / amev_t;
							}
							else
							{
								//  if(m_rand[aa][i][j][sdmap[m][n][i]][0] == 0) mev[sdmap[m][n][i]][i][j][0] = 0;
								//  else mev[sdmap[m][n][i]][i][j][0] = m_rand[aa][i][j][sdmap[m][n][i]][0] * mev_tmp;
								for (jj = 0; jj <= jm[aa][i][sdmap[m][n][i]]; jj++)
								{
									if ((int)buf[4] == b4[aa][i][jj][sdmap[m][n][i]])
									{
										if (m_rand[aa][i][jj][sdmap[m][n][i]][0] == 0)
											mev[sdmap[m][n][i]][i][jj][0] = 0;
										else
											mev[sdmap[m][n][i]][i][jj][0] = m_rand[aa][i][jj][sdmap[m][n][i]][0] * amev_t;
									}
									printf("0  nsd:%d, buf4:%d, b4:%d, mev_tmp:%f, m_rand:%f, mev0:%f\n", j, (int)buf[4], b4[aa][i][jj][sdmap[m][n][i]], amev_t, m_rand[aa][i][jj][sdmap[m][n][i]][0], mev[sdmap[m][n][i]][i][jj][0]);
								}
							}
...
/*if(np==0){
  ...
  poisson_tmp = m_rand[aa][i][j][sdmap[m][n][i]][1];
}*/
							/*		      if (poisson_tmp > 0.)
			poisson_tmp *= 1.+ NPE2FADCE[1]/sqrtf(poisson_tmp)*
			  randnum[1];
...
*/
							...
							else
							{
								for (jj = 0; jj <= jm[aa][i][sdmap[m][n][i]]; jj++)
								{
									if ((int)buf[4] == b4[aa][i][jj][sdmap[m][n][i]])
									{
										if (m_rand[aa][i][jj][sdmap[m][n][i]][1] == 0)
											mev[sdmap[m][n][i]][i][jj][1] = 0;
										else
											mev[sdmap[m][n][i]][i][jj][1] = m_rand[aa][i][jj][sdmap[m][n][i]][1] * mev_tmp;
									}
									printf("1 nsd:%d, m_rand:%f, mev1:%f\n", j, m_rand[aa][i][jj][sdmap[m][n][i]][1], mev[sdmap[m][n][i]][i][jj][1]);
								}
							}
							...
							if (np == 0)
							{
								...
								time[sdmap[m][n][i]][i][j] = (tmptime * DT -
															  (int)(sdht[sdmap[m][n][i]][i] *
																	pztmp / CSPEED2)) /
															 DT;
								//  printf("np:%d, nevt_global:%d, nevt:%d, timebin:%d, x:%d, y:%d, sdmap:%d, tmptime:%d, time:%d\n",  np, aa, i, j, m, n, sdmap[m][n][i], tmptime, time[sdmap[m][n][i]][i][j]);
							...
							{
								////  mev[sdmap[m][n][i]][i][j][0] = m_rand[aa][i][j][sdmap[m][n][i]][0];
								////  mev[sdmap[m][n][i]][i][j][1] = m_rand[aa][i][j][sdmap[m][n][i]][1];
								//  if(time[sdmap[m][n][i]][i][j] > -1000000) time[sdmap[m][n][i]][i][j] = t_rand[aa][i][j][sdmap[m][n][i]];
								for (jj = 0; jj <= jm[aa][i][sdmap[m][n][i]]; jj++)
								{
									if ((int)buf[4] == b4[aa][i][jj][sdmap[m][n][i]])
										time[sdmap[m][n][i]][i][jj] = t_rand[aa][i][jj][sdmap[m][n][i]];
									printf(" nsd:%d, t_rand:%d, time:%d\n", j, t_rand[aa][i][jj][sdmap[m][n][i]], (int)time[sdmap[m][n][i]][i][jj]);
								}
							}
			...
			for (i = 0; i < nEvent; i++)
			{
				for (j = 0; j < NCOUNTERS; j++)
				{
					//  printf("np:%d, nevt_global:%d, nevt:%d, ndet:%d, deadtime:%d, time:%d\n",  np, aa, i, j, deadtime[(int)showertime[i]][j], time[j][i][0]);
					if (np == 0)
					{
							...
							time[j][i][0] > -1000000)
						{
							...
							{
								gaussf(randnum);
								crand[aa][i][j][m][0] = randnum[0];
								crand[aa][i][j][m][1] = randnum[1];
								...
								//  if(m<40) printf("np:%d, nevt_global:%d, nevt:%d, ndet:%d, crand1:%f, uped:%f\n", np, aa, i, j, randnum[0], fadc[j][i][m][0]);
								//  printf("---ped--- %d %d %d %f %f %f %f\n", i, j, m, fadc[j][i][m][0], fadc_ped[(int)showertime[i]][j][0], fadc_noise[(int)showertime[i]][j][0], randnum[0] );
							}
							k = 0;
							printf("nevt ndet time_1 time_2 fadc mev one_mip one_mev\n");
							while (time[j][i][k] > -1000000 && k < TMAX)
							{
								printf("--- hit --- nsd:%d, nevt:%d, k:%d, time:%d\n", j, i, k, time[j][i][k]);
								for (m = 0; m < RESP_WIDTH; m++)
								{
										...
										m + time[j][i][k] + FADC_START < FADC_TOT)
									{
										...
										printf("%d %d %d %d %f %f %f %f\n", i, j, m + time[j][i][k] + FADC_START, k, fadc[j][i][m + time[j][i][k] + FADC_START][0], mev[j][i][k][0], one_mip[m], one_mev[(int)showertime[i]][j][0]);
										faflg[m][k] = 1;
									}
								}
								k++;
													   ...
													   ((fadc[j][i][m][1] -
														...
														expf(-(ATTEN *
															   (fadc[j][i][m][1] -
																sat[(int)showertime[i]][j][1]) /
															   FADC_NONLIN2)));
								if (fadc[j][i][m][0] > FADC_MAX)
									fadc[j][i][m][0] =
										FADC_MAX;
								if (fadc[j][i][m][1] > FADC_MAX)
									fadc[j][i][m][1] =
										FADC_MAX;
								for (k = 0; k < TMAX; k++)
								{
									//    if(faflg[m][k] ==1) printf("///  %d %d %d %d %f %f %f %f\n", i, j, m+time[j][i][k]+FADC_START, k, fadc[j][i][m+time[j][i][k]+FADC_START][0], mev[j][i][k][0], one_mip[m], one_mev[(int)showertime[i]][j][0] );
									faflg[m][k] = 0;
								}
							}
							trigsum[0] = 0.;
						  ...
						  trigtable[j][i][wfnum]); */
							      ...
							      dtrigtable[j][i][wfnum2]); */
									...
									for (m = trigtable[j][i][wfnum] + 97;
										...
										for (n = 0; n < 2; n++)
											trigsum[n] += rint(fadc[j][i][m][n]);
									//    atable[aa][i][j][m] = trigtable[j][i][wfnum];
									wfnum++;
								}
								for (n = 0; n < 2; n++)
								{
					...
					else
					{
						...
						if (sdflg[aa][i][j] == 1)
						{
							if (np == 5)
							{
								for (m = 0; m < FADC_TOT; m++)
								{
									randnum[0] = crand[aa][i][j][m][0];
									randnum[1] = crand[aa][i][j][m][1];
									fadc[j][i][m][0] += fadc_ped[(int)showertime[i]][j][0] +
														fadc_noise[(int)showertime[i]][j][0] * randnum[0];
									fadc[j][i][m][1] += fadc_ped[(int)showertime[i]][j][1] +
														fadc_noise[(int)showertime[i]][j][1] * randnum[1];
										...
										while (dtrigtable[m][i][r] > 0 && r < FADC_TOT / 2)
										{
											if (((abs(row[j] - row[m]) == 1 &&
												...
												abs(dtrigtable[k][i][q] / 50 -
													dtrigtable[m][i][r] / 50) < 8)
											{
												dtrigtime_tmp = (dtrigtable[j][i][p] + dtrigtable[k][i][q] + dtrigtable[m][i][r]) / 3.;
												if (dtrigtime_tmp <
	  ...
	  for ( j = 0; j < NCOUNTERS; j++ )
	    ...
	    printf("\n\n"); */
			...
			for (i = 0; i < nEvent; i++)
			{
				...
				if (np == 0)
				{
					nwf[aa][i] = rusdraw_.nofwf;
				}
				else
					rusdraw_.nofwf = nwf[aa][i];
				put2rusdraw();
				if (np == 0)
				{
					rusdraw_.event_num = (int)lrand48();
					en[aa][i] = rusdraw_.event_num;
				}
				else
					rusdraw_.event_num = en[aa][i];
				...
				{
					for (kk = 0; kk < 3; kk++)
						st[aa][i][kk] = secttime[i][kk];
				}
				else
				{
					for (kk = 0; kk < 3; kk++)
						secttime[i][kk] = st[aa][i][kk];
				}
				////
					...
					for (j = 0; j < NCOUNTERS; j++)
					{
						...
						if (np == 0)
						{
							while (trigtable[j][i][q] > 0 && q < FADC_TOT / 2)
							{
								...
								{
									trflg[aa][i][j][q] = 1;
									atable[aa][i][j][q] = trigtable[j][i][q];
									printf("---trflg0---  np:%d, nevt_global:%d, nevt:%d, nsd:%d, q:%d, qq:%d, k:%d, eventnum:%d, yymmdd:%d\n", np, aa, i, j, q, qq, k, rusdraw_.event_num, rusdraw_.yymmdd);
									rusdraw_.xxyy[k] = sdcoor[j][0];
									rusdraw_.clkcnt[k] = trigtable[j][i][q] - 30 + time_offset;
									printf("xxyy: %d, clk: %d trig:%d\n", rusdraw_.xxyy[k], rusdraw_.clkcnt[k], trigtable[j][i][q]);
									printf("fadc:\nm n fadc\n");
									for (m = 0; m < FADC_LENGTH; m++)
									{
										for (n = 0; n < 2; n++)
										{
											...
											rusdraw_.fadcti[k][1 - n] +=
												rusdraw_.fadc[k][1 - n][m];
											printf("%d %d %d\n", m, n, rusdraw_.fadc[k][1 - n][m]);
										}
									}
									printf("\nfadcti:\nm n fadcti\n");
									for (n = 0; n < 2; n++)
									{
										printf("%d %d %d\n", m, n, rusdraw_.fadcti[k][1 - n]);
									}
									//	  printf("fadc: %d, clf: %f\n", rusdraw_.xxyy[k], rusdraw_.clkcnt[k]);
									rusdraw_.wf_id[k] = qq;
									rusdraw_.fadcav[k][0] = (int)rintf(8. * fadc_ped[(int)showertime[i]][j][1]);
									rusdraw_.fadcav[k][1] = (int)rintf(8. * fadc_ped[(int)showertime[i]][j][0]);
									...
									rusdraw_.pchmip[k][0] = (int)rintf(12. * fadc_ped[(int)showertime[i]][j][1] +
																	   mip[(int)showertime[i]][j][1]);
									rusdraw_.pchmip[k][1] = (int)rintf(12. * fadc_ped[(int)showertime[i]][j][0] +
																	   mip[(int)showertime[i]][j][0]);
									rusdraw_.lhpchmip[k][0] = (int)rintf(12. * fadc_ped[(int)showertime[i]][j][1] +
																		 0.5 * mip[(int)showertime[i]][j][1]);
									rusdraw_.lhpchmip[k][1] = (int)rintf(12. * fadc_ped[(int)showertime[i]][j][0] +
																		 0.5 * mip[(int)showertime[i]][j][0]);
									rusdraw_.rhpchmip[k][0] = (int)rintf(12. * fadc_ped[(int)showertime[i]][j][1] +
																		 1.5 * mip[(int)showertime[i]][j][1]);
									rusdraw_.rhpchmip[k][1] = (int)rintf(12. * fadc_ped[(int)showertime[i]][j][0] +
						...
						if (np > 0)
						{
							while (q < FADC_TOT / 2)
							{
								...
								{
									//        qq=0;
									//        while ( qq < qqmax[aa][i][j][q] ){
									//        if(trflg[aa][i][j][q] > 0){

									trigtable[j][i][q] = atable[aa][i][j][q];

									printf("---  trflg  ---  np:%d, nevt_global:%d, nevt:%d, nsd:%d, q:%d, qq:%d, k:%d, eventnum:%d, yymmdd:%d\n", np, aa, i, j, q, qq, k, rusdraw_.event_num, rusdraw_.yymmdd);
									rusdraw_.xxyy[k] = sdcoor[j][0];
									rusdraw_.clkcnt[k] = trigtable[j][i][q] - 30 + time_offset;
									printf("xxyy: %d, clk: %d, trig:%d\n", rusdraw_.xxyy[k], rusdraw_.clkcnt[k], trigtable[j][i][q]);
									printf("fadc:\nm n fadc\n");
